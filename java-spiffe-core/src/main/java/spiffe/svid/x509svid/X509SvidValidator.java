...
public class X509SvidValidator {
    ...
    public static Result<Boolean, String> verifyChain(
        ...
        if (!x509Bundle.isPresent()) {
            return Result.error(String.format("No X509 Bundle found for the Trust Domain %s", trustDomain));
        }

        val result = CertificateUtils.validate(chain, x509Bundle.get().getX509Roots());
        if (result.isError()) {
     ...
     *
     * @return an {@code Ok} with true if the SPIFFE ID is in the list,
     * an {@code Error} containing en error message if the SPIFFE ID is not in the list
